...
class TestApplication(unittest.TestCase):
    def test_execution_api_service_returns_plot_path(self):
        request=SimulationRequest(side="SELL",strategy="adaptive",parent_quantity=.001,duration_seconds=30,
    ...
    def test_request_validation_rejects_invalid_quantity(self):
        with self.assertRaises(ValueError):SimulationRequest(side="SELL",strategy="twap",parent_quantity=0,duration_seconds=30,volatility=2,spread=.1,arrival_rate=.5,displayed_depth=.3,seed=1)

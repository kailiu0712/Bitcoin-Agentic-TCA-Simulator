...
def load_calibration_target(path="outputs/baseline_market/real_stylized_facts/pooled_targets.json"):
    ...
    if target_path.exists(): return json.loads(target_path.read_text())["calibration"]
    # Legacy output bundles may be intentionally pruned. Keep the loader
    # callable for diagnostics; P0/P1 runs use config/default.yaml targets.
    return {"n_trades": 0}
...
def suggest(trial,base):
    ...
    x.update(market_order_arrival_rate=trial.suggest_float("market_order_arrival_rate",.25,.75),
        ...
        order_flow_long_memory_probability=trial.suggest_float("order_flow_long_memory_probability",.25,.98),
        order_flow_long_memory_exponent=trial.suggest_float("order_flow_long_memory_exponent",.25,.95),
        ...
        market_maker_queue_imbalance_response=trial.suggest_float("market_maker_queue_imbalance_response",-2.0,2.0),
        ...
        market_maker_spread_sensitivity=trial.suggest_float("market_maker_spread_sensitivity",.0001,3.0,log=True),
        ...
        order_flow_hawkes_branching=trial.suggest_float("order_flow_hawkes_branching",.35,.95),
        order_flow_hawkes_decay=trial.suggest_float("order_flow_hawkes_decay",.001,1.5,log=True),
        large_order_probability=trial.suggest_float("large_order_probability",.002,.03),
        large_order_multiplier=trial.suggest_float("large_order_multiplier",10.0,250.0,log=True),
        large_order_pareto=trial.suggest_float("large_order_pareto",1.05,3.0))
    return x


def main():
    ap=argparse.ArgumentParser();ap.add_argument("--config",default="config/default.yaml");ap.add_argument("--targets",default="outputs/baseline_market/real_stylized_facts/pooled_targets.json");ap.add_argument("--output-dir",default="outputs/calibration");ap.add_argument("--trials",type=int);ap.add_argument("--duration-seconds",type=float);ap.add_argument("--seeds-per-trial",type=int);ap.add_argument("--p0p1-only",action="store_true")
    args=ap.parse_args();full=yaml.safe_load(Path(args.config).read_text());base=full["simulation"];ccfg=full["calibration"];target=load_calibration_target(args.targets);target.update(ccfg.get("priority_targets", {}));
    if args.p0p1_only:
        target={k:v for k,v in target.items() if k in ccfg.get("priority_targets", {})}
    out=Path(args.output_dir);out.mkdir(parents=True,exist_ok=True)
    run_duration=args.duration_seconds or base["duration_seconds"]
    run_seeds=args.seeds_per_trial or ccfg["seeds_per_trial"]
    started=time.perf_counter();sampler=optuna.samplers.TPESampler(seed=base["seed"],n_startup_trials=min(15, max(3, (args.trials or ccfg["trials"]) // 2)),multivariate=True);study=optuna.create_study(direction="minimize",sampler=sampler)
    initial_cfg=dict(base)
    def objective(trial):
        cfg=dict(initial_cfg if trial.number==0 else suggest(trial,base));cfg["duration_seconds"]=run_duration;losses=[];all_rows=[]
        for seed_index in range(run_seeds):

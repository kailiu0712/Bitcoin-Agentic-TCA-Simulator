...
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--config",default="outputs/calibration/best_config.yaml");ap.add_argument("--base-config",default="config/default.yaml");ap.add_argument("--targets",default="outputs/baseline_market/real_stylized_facts/pooled_targets.json");ap.add_argument("--output-dir",default="outputs/validation");ap.add_argument("--seeds",type=int,default=10)
    args=ap.parse_args();base=yaml.safe_load(Path(args.base_config).read_text());cfg=yaml.safe_load(Path(args.config).read_text())["simulation"];targets=json.loads(Path(args.targets).read_text());weights=base["calibration"]["weights"];out=Path(args.output_dir);out.mkdir(parents=True,exist_ok=True)
    ...
    variability=json.loads(Path("outputs/baseline_market/real_stylized_facts/daily_variability.json").read_text());scalar_rows=[]
    for period in ("calibration","validation"):
    ...
    key=wide.set_index("metric");gate_metrics=["spread_distribution","relative_spread_distribution","trade_size_distribution","interarrival_distribution","sign_acf_curve","response_curve","clock_response_curve","size_impact_curve","flow_impact_curve","return_volatility","bid_depth_distribution","ask_depth_distribution"]
